...
public class KdTreePointST<Value> implements PointST<Value> {
    ...
    public KdTreePointST() {
        // Intiliazes root and n to be null & 0
        this.root = null;
        this.n = 0;
    }
    ...
    public void put(Point2D p, Value value) {
        // Creates a rect with all sides negative infinity 
    ...
    public Iterable<Point2D> points() {
        // 2 Linked queues are made one being 2D and one Node
        LinkedQueue<Point2D> ans = new LinkedQueue<Point2D>();
        LinkedQueue<Node> algorithm = new LinkedQueue<Node>();
        // Root is added to algorthirm 
        algorithm.enqueue(root);
        // Goes through algorithm and is lb or rt is not null adds it to ans queue
    ...
    private Node put(Node x, Point2D p, Value value, RectHV rect, boolean lr) {
        ...
        // If x.p equal p set x.value to be value and return the x
        if (x.p.equals(p)) {
            x.value = value;
            return x;
        }
